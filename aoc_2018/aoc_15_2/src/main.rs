            ...
            .fold(vec![], |mut out, line| {
                ...
                out.push(line.chars().fold(vec![], |mut out, c| {
                    out.push(match c {
                        ...
                        'G' => {
                            fighters.push(Fighter::new(false, xcnt, ycnt, fcnt, 3));
                            fcnt += 1;
                            Node::new(Tile::Fighter, Some(fcnt - 1))
                        }
                        'E' => {
                            fighters.push(Fighter::new(true, xcnt, ycnt, fcnt, power));
                            fcnt += 1;
                            Node::new(Tile::Fighter, Some(fcnt - 1))
                        }
            ...
            'floop: for i in ids.iter().map(|i| i.id) {
                ...
                if !targets.is_empty() {
                    ...
                    if fighters[targets[0].0].hp <= 0 {
                        if fighters[targets[0].0].is_elve {
                            dead_elves += 1;
                        }
...
impl Fighter {
    fn new(is_elve: bool, posx: usize, posy: usize, id: usize, ap: i32) -> Self {
        Fighter {
            hp: 200,
            attack_power: ap,
            is_elve: is_elve,
